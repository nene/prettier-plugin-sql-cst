...
- :white_check_mark: :wrench: `TRUNCATE` -> `TRUNCATE TABLE` (standard syntax.)
- :white_check_mark: :wrench: `MERGE` -> `MERGE INTO` (standard syntax.)
- :white_check_mark: :wrench: `CREATE TEMP TABLE` -> `CREATE TEMPORARY TABLE` (standard syntax)
- `ADD` -> `ADD COLUMN` (more commonly supported)
